...
//! note. `Next` ships one bullet-less checkbox with a TRAILING SPACE:
//! `markdown::checkbox` accepts the bare form, and the space is load-bearing
//! because `state::is_blank` compares the buffer to this const with `==`.
